...
Please extract and explain this in simple {language_name}."""
    ...
    # Try configured model and fallback models in sequence if model_not_found or API error
    tried_models = []
    last_exception = None

    for model_name in dict.fromkeys(FALLBACK_MODELS):  # deduplicate while preserving order
        try:
            ...
            if raw_output.startswith("```"):
                ...
                # strip code fence markers like ```json ... ```
    ...
    raise RuntimeError(f"All attempted Groq models ({', '.join(tried_models)}) failed: {last_exception}")
